...
const Cat = () => {
  const catData = animalData.filter((animal) => animal.type === "kat");

  const [filter, setFilter] = useState("alles");
  const [filteredAnimals, setFilteredAnimals] = useState(catData);

  const handleFilterChange = (event) => {
    const selectedFilter = event.target.value;
    setFilter(selectedFilter);

    let animalsToShow = [...catData]; 
    ...
    <>
      <section className="adopt-container">
        <article className="width b_flex">
          <h2>Katten te Adoptie</h2>
          <p>
            Hier kun je al onze katten vinden. Van jong
